...
function headingIdSet(root){return new Set([...els(root,'h2'),...els(root,'h3')].map(h=>attr(h,'id')).filter(Boolean));}
function headingAnchorLinks(root,n){const ids=headingIdSet(root); return descendants(n,'a').filter(a=>{const h=attr(a,'href'); return h.startsWith('#')&&ids.has(h.slice(1));});}
function tocLabel(n){return /目次|INDEX|TOC|table-of-contents|outline|この章でわかること|このセクションでわかること|この章の内容|この記事でわかること/i.test([attr(n,'class'),attr(n,'id'),attr(n,'aria-label'),visible(n)].join(' '));}
function isTocContainer(root,n){const links=headingAnchorLinks(root,n); if(!links.length)return false; const listLike=['ul','ol','nav'].includes(n.tagName); const label=tocLabel(n); return (links.length>=2&&(listLike||label||hasClass(n,'swell-block-capbox')||/box|summary|index|toc/i.test(attr(n,'class')))) || (links.length>=1&&label);}
function linkListTargets(root,n){return headingAnchorLinks(root,n).map(a=>attr(a,'href'));}
function isManualTocNav(root,n){if(n.tagName!=='nav')return false; return isTocContainer(root,n);}
...
export function validateNoManualToc(html,{approvedOutline=[],metadata={}}={}){
 ...
 if(approvedOutline.length){const approved=approvedOutline.map(h=>({level:Number(h.level),text:norm(h.text)})).filter(h=>h.level&&h.text); const actual=hs.map(h=>({level:h.level,text:h.text})); const expected=approved.some(h=>h.level===3)?actual:actual.filter(h=>h.level===2); if(JSON.stringify(expected)!==JSON.stringify(approved))errors.push('承認済みH2・H3の件数、順序、親子関係が異なります'); }
